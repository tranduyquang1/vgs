...
class EmailBccModel extends AdminModel
{
    public function __construct()
    {
        $this->table = 'email_bcc';
        $this->folderUpload = '';
    ...
    public function getItem($params = null, $options = null)
    {
        ...
        if ($options['task'] == 'by-template') {
            $result = self::select()->where('status', '=', 'active')->where('template', 'like', '%"' . $params['templateId'] . '"%')->get();
        }
