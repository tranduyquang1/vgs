...
class Template
{
    ...
    { // $currentFilterStatus active inactive all
        ...
        if (count($itemsStatusCount) > 0) {
            ...
            foreach ($itemsStatusCount as $item) {  // $item = [count,status]
                ...
                $class = ($currentFilterStatus == strval($statusValue)) ? 'btn-danger' : 'btn-info';
    ...
    /**
     * Show button filter (tuition_status)
     * @param $controllerName
     * @param $itemsStatusCount
     * @param $currentFilterStatus
     * @param $paramsSearch
     ...
     * @return string|null
     */
    public static function showButtonFilterV2($controllerName, $itemsStatusCount, $currentFilterStatus, $paramsSearch, $type)
    { // $currentFilterStatus active inactive all
        $xhtml = null;
        $tmplStatus = Config::get('zvn.template.' . $type);
        ...
        if (count($itemsStatusCount) > 0) {
            ...
            foreach ($itemsStatusCount as $item) {  // $item = [count,status]
                $statusValue = $item['tuition_status'];  // active inactive block
                $statusValue = array_key_exists($statusValue, $tmplStatus) ? $statusValue : 'default';
    ...
    public static function showItemButton($controllerName, $id, $statusValue, $type)
    {
        ...
        $link = route("admin.$controllerName.status", ['status' => $statusValue, 'id' => $id]);

        $xhtml = sprintf(
            '<button type="button" data-link="%s" class="m-0 btn btn-sm btn-round %s-ajax %s">%s</button>', $link, $type, $currentTemplateStatus['class'], $currentTemplateStatus['name']);
        return $xhtml;
    }
    ...
    public static function showIsFullButton($controllerName, $id, $statusValue, $type)
    {
        ...
        $xhtml = sprintf(
            '<button type="button" data-link="%s" class="btn btn-sm btn-round %s-ajax %s">%s</button>', $link, $type, $currentTemplateStatus['class'], $currentTemplateStatus['name']);
        return $xhtml;
    }
    ...
    public static function showItemIsHome($controllerName, $id, $isHomeValue)
    {
        ...
        $xhtml = sprintf(
            '<button type="button" data-link="%s" class="btn btn-sm btn-round isHome-ajax %s">%s</button>', $link, $currentTemplateIsHome['class'], $currentTemplateIsHome['name']);
        return $xhtml;
    }
    ...
    public static function showItemConfigProduct($controllerName, $id, $config, $statusValue)
    {
        ...
        $xhtml = sprintf(
            '<button type="button" data-link="%s" data-config="%s" class="btn btn-sm btn-round config-ajax %s">%s</button>', $link, $config, $currentTemplateStatus['class'], $currentTemplateStatus['name']);
        return $xhtml;
    }
    ...
    /**
     * Create button send mail
     *
     * @param $url
     * @param null $options
     * @return string
     */
    public static function showButtonSendMail($url, $options = null)
    {
        $btnName = isset($options['btnName']) ? $options['btnName'] : 'Gửi mail';
        $xhtml = sprintf('<button type="button" data-link="%s" class="btn btn-sm btn-round btn-primary btn-mail %s">%s <i class="fa fa-spin fa-refresh ml-1 d-none"></i></button>',
            $url, $options['class'], $btnName);
    ...
    public static function showItemThumbFull($thumb, $thumbAlt) {
        return sprintf('<img src="%s" alt="%s" class="zvn-thumb">', asset($thumb), $thumbAlt);
    }
    ...
    /**
     * Show image thumb by url
     * @param $host
     * @param $thumbName
     * @param $thumbAlt
     * @return string
     */
    public static function showItemThumbbyUrl($host, $thumbName, $thumbAlt)
    {
        $xhtml = sprintf(
            '<img src="%s" alt="%s" class="zvn-thumb">', $host . $thumbName, $thumbAlt);
        return $xhtml;
    }
     ...
     * @return string
     */
    public static function showItemSelectObject($controllerName, $id, $arrValue, $displayValue, $fieldName, $idCourse = null)
    {
        $link = ($id == null) ? route($controllerName . '/attribute') : route($controllerName . '/attribute', ['id' => $id]);

        $xhtml = sprintf('<select name="select_change_attr_supporter" data-field="%s" data-id-course="%s" data-id-object="%s" data-url="%s" class="form-control select-ajax">', $fieldName, $idCourse, $id, $link);
        $xhtml .= '<option value="default"> - Chọn giá trị - </option>';
        foreach ($arrValue as $key => $value) {
            $xhtmlSelected = '';
            if ($key == $displayValue) $xhtmlSelected = 'selected="selected"';
     ...
     * @return string
     */
    public static function showSelectFilterCustom($field_name, $values, $selected = 'default', $showConfig = true)
    {
        $xhtml = sprintf('<select name="select_filter_custom" class="form-control mr-1" style="width: auto" data-field="%s">', $field_name);
        foreach ($values as $key => $value) {
            $xhtmlSelected = '';
            if ((string)$key === $selected) {
                $xhtmlSelected = 'selected="selected"';
    ...
    /**
     * Show button with copy
     * @param $link
     * @return string
     */
    public static function showButtonCoppy($link)
    {
        $xhtml = '
            <a href="' . $link . '" class="btn btn-sm btn-primary btn-reply" target="_blank"><i class="fa fa-reply"></i> Reply</a>
            <a data-link="' . $link . '" class="btn btn-sm btn-info copy-link">Copy Link</a>
        ';
            ...
            <div class="col-md-8 col-sm-10 col-xs-12">
                <div class="d-flex algin-items-center">
                    <div class="input-group" style="margin-right: 10px">
                       <span class="input-group-btn">
                         <a data-input="image_main_' . $name . '" data-preview="' . $name . '" class="btn btn-primary lfm">
                           <i class="fa fa-picture-o"></i> Chọn ảnh
                         </a>
                       </span>
                       <input id="image_main_' . $name . '" class="form-control" type="hidden" name="' . $name . '" value="' . $value . '">
                    </div>
                    <img id="' . $name . '" src="' . $src . '" class="mb-4 img-fluid image-preview" style="max-height:60px;">
               </div>
            </div>
        </div>';
    }
     ...
     * @return false|string
     */
    public static function showLongTime($dateTime)
    {
        return date_format(date_create($dateTime), Config::get('zvn.format.long_time'));
    }
    ...
    public static function fitImageForDisplay($image)
    {
        $path = 'https://cdn.golfnews.vn' . $image;
        $img = Image::make($path);
        $img->fit(1280, 720);
