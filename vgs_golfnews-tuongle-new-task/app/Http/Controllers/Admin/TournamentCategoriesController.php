...
use App\Http\Requests\TournamentCategoriesSaveRequest;
...
class TournamentCategoriesController extends Controller
{
    public $controllerName = 'tournament-categories';
    public $pathViewController = 'admin.pages.tournament-categories.';
    protected $model;
    protected $params = [];

    public function __construct(TournamentCategories $model)
    {
        $this->model = $model;
    ...
    public function ListCategoriesIndex(Request $request){
        Gate::forUser($this->user)->authorize("tournament-categories-index", $this->model);
    ...
    {
        Gate::forUser($this->user)->authorize('tournament-categories-form', $this->model);
        $item = null;
        if ($request->id) {
    ...
    public function ListCategoriesSave(TournamentCategoriesSaveRequest $request)
    {   
        Gate::forUser($this->user)->authorize('tournament-categories-save', $this->model);
        if ($request->id) {
    ...
    public function ListCategoriesDelete(Request $request) {
        Gate::forUser($this->user)->authorize('tournament-categories-delete', $this->model);
        $item = $this->model->find($request->id);
